    ...
    <div class="col-md-9 ">
        <div class="card">
            ...
            <div class="body">
                ...
                <p><span class="text-primary">Merchant Currency Code : </span>{{$merchant->currency && $merchant->currency->code}}</p>
                <p><span class="text-primary">Merchant Currency Symbol : </span>{{$merchant->currency && $merchant->currency->symbol}}</p>
                <p><span class="text-primary">Merchant Request URL : </span>{{url('/')}}/{{app()->getLocale()}}/purchase/link</p>
                <p><span class="text-primary">Merchant Transaction Status URL : </span>{{url('/')}}/{{app()->getLocale()}}/request/status</p>
        ...
        <div class="card">
            ...
            <div class="body">
                ...
                <div class="alert alert-info mb-5">
                    <p>Send a CURL POST request that contains <strong><small>(string) => </small>'merchant_key'</strong>, <strong><small>(array) => </small>'invoice'</strong>, and <strong><small>(string) => </small> 'currency_code'</strong> variables to <kbd>{{url('/')}}/{{app()->getLocale()}}/purchase/link</kbd>. As a response you get an array containing the link to the checkout page
                    </p>
...
$invoice = json_encode($invoice);
    ...
    $post = array(
        ...
        'invoice'=> $invoice,
        'currency_code' =>  '{{$merchant->currency && $merchant->currency->code}}'
        );

        

        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, '{{url('/')}}/{{app()->getLocale()}}/purchase/link' );
        ...
        if (isset($_GET['token']) and !is_null($_GET['token'])) {
            ...
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, '{{url('/')}}/{{app()->getLocale()}}/request/status' );
            curl_setopt($ch, CURLOPT_POST, true);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
            curl_setopt($ch, CURLOPT_POSTFIELDS, $post);
